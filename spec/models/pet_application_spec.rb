...
RSpec.describe PetApplication, type: :model do
  ...
  it { should belong_to :application}
	...
	describe 'class methods' do
		...
		let!(:petapp_2) { PetApplication.create!(pet: pet_1, application: app_2, approval: 2)}
		let!(:petapp_3) { PetApplication.create!(pet: pet_2, application: app_1, approval: 1)}
		let!(:petapp_4) { PetApplication.create!(pet: pet_2, application: app_2, approval: 2)}
      ...
      it 'returns true if all applications are all rejected' do
        expect(PetApplication.all_rejected?(app_1.id)).to eq(false)
				expect(PetApplication.all_rejected?(app_2.id)).to eq(true)
      end
    end
	end
end
